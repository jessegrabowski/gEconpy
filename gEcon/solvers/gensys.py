...
def gensys(g0: ArrayLike,
           ...
           realsmall: Optional[float] = 1e-8) -> Tuple:
    """
    Christopher Sim's gensys, http://sims.princeton.edu/yftp/gensys/mfiles/gensys.m
    :param g0: ArrayLike, coefficient matrix of the dynamic system corresponding to the time-t variables
    :param g1: ArrayLike, coefficient matrix of the dynamic system corresponding to the time t-1 variables
    :param c: ArrayLike, vector of constant terms
    :param psi: ArrayLike, coefficient matrix of the dynamic system corresponding to the exogenous shock terms
    :param pi: ArrayLike, coefficient matrix of the dynamic system corresponding to the endogenously determined
        expectational errors.
    :param div: Optional, float, the largest positive value for which an eigenvalue is considered a stable root.
    :param realsmall: float, default = 1e-8. an arbitrarily small number
    :return: Tuple of 9 elements:
        G_1: ArrayLike, coefficient matrix associated with t-1 terms in the policy function
        C: ArrayLike, vector of constant terms in the policy function
        impact: ArrayLike, coefficient matrix associated with exogenous shocks in the policy function
        f_mat, f_wt, y_wt, gev, loose: together represent the coefficient matrix associated with correlated shocks in
                                       the policy function (see docstring). Equals 0 if shocks are IID.

        eu: A list of three integers encoding the solver's status (see docstring), and the number of loose
            enodgenous variables.
    ...
    TODO: Write numba and Aesara implementations of this. The big stumbling block is the QZ decomposition has no
        CUDA implementation in LAPACK. There are other solutions algorithms though -- check the Dynare docs.
    """
    eu = [0, 0, 0]
    ...
    div, n_unstable, zxz = determine_n_unstable(A, B, div, realsmall)
    ...
    if n_unstable == 0:
        ...
        u_eta, v_eta, d_eta, big_ev = build_u_v_d(eta_wt, realsmall)
    ...
    if n_unstable == n:
        ...
        eta_wt_1 = Q1 @ pi
        u_eta_1, v_eta_1, d_eta_1, big_ev = build_u_v_d(eta_wt_1, realsmall)
    ...
    if 0 in v_eta_1.shape:
        ...
        n_loose = (np.abs(np.diagonal(dl)) > (realsmall * n)).sum()
        eu[2] = n_loose
        unique = n_loose == 0
