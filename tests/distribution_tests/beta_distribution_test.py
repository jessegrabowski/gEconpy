...
class TestBetaDistributionParser(unittest.TestCase):
    def setUp(self):
        ...
        self.parse_shape_parameter_1 = partial(
            self.parser._parse_parameter,
            canon_name=self.parser.shape_param_name_1,
            aliases=BETA_SHAPE_ALIASES_1,
        )
        self.parse_shape_parameter_2 = partial(
            self.parser._parse_parameter,
    ...
    def test_typo_in_alpha(self):
        ...
        self.assertWarns(IgnoredCloseMatchWarning, self.parse_shape_parameter_1, param_dict)
