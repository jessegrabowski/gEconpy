...
class TestNormalDistributionParser(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.parse_lower_bound = partial(self.parser._parse_parameter,
                                         canon_name='a',
                                         aliases=LOWER_BOUND_ALIASES)
        self.parse_upper_bound = partial(self.parser._parse_parameter,
                                         canon_name='b',
    ...
    def test_typo_in_bounds(self):
        ...
        self.assertWarns(IgnoredCloseMatchWarning,
                         self.parse_upper_bound,
                         param_dict)

        param_dict = {'loc': 1, 'scale': 2, 'lower___bound': 3}
        self.assertWarns(IgnoredCloseMatchWarning,
                         self.parse_lower_bound,
                         param_dict)
