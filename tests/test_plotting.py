...
# MPLBACKEND=Agg is set under [tool.pytest.ini_options] in pyproject.toml, so every figure renders off-screen.
...
# Each fixture builds a plotting input once per session, so the tests below sweep argument combinations through
# them without recompiling models.
...
def sensitivity_data(one_block_model):
    # eigenvalue_sensitivity forwards its own verbose into model.steady_state, so it must not also appear in
    # steady_state_kwargs.
...
def fake_posterior_idata():
    """Posterior DataTree with three scalar parameters and a shock matrix, built from random arrays."""
...
def test_plot_posterior_with_prior_accepts_one_shot_iterable(fake_posterior_idata):
    # A generator must survive being measured with len() and iterated more than once.
    fig = plot_posterior_with_prior(
        fake_posterior_idata,
