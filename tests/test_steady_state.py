...
class SteadyStateSolverTests(unittest.TestCase):
    ...
    def test_create_jacobian(self):
        ...
        f_jac = self.solver._build_jacobian(
            diff_variables=["x", "y"], additional_inputs=["a"], equations=equations
        )
        expected_result = np.array([[4.0, 0.0], [1.0, 2.0]])

        self.assertTrue(
            np.allclose(
                f_jac(args=[], kwargs={"a": 1.0, "x": 2.0, "y": 1.0}), expected_result
            )
        )
...
class FullModelSolutionProvidedTest(unittest.TestCase):
    def setUp(self):
        self.model = gEconModel(
            "Test GCNs/One_Block_Simple_1_w_Steady_State.gcn", verbose=False
        )
